...
from cat_num import conversion_cat_num
from ordinal import conversion_cat_num1
...
class SERVICE:
    ...
    def cat_to_num(self):
        try:
            ...
            self.X_train_cat, self.X_test_cat = conversion_cat_num1(self.X_train_cat, self.X_test_cat)
            self.X_train_num,self.X_test_num = self.variable_tf(self.X_train_num,self.X_test_num)
            sns.boxplot(self.X_train_num)
            ...
            logger.info(f"After removing {self.training_ind_data.columns}")
    ...
    def scaling_data(self):
        try:
            ...
            logger.info(f"info : {self.training_ind_data_up.head(1)}")
            sc = StandardScaler()
            ...
            model_training(self.scaled_training_inde_cols,self.y_train_up,self.scaled_test_inde_cols,self.y_test)
            selecting_best_model(self.scaled_training_inde_cols, self.y_train_up, self.scaled_test_inde_cols,
                                 self.y_test)
